...
export default async function ProfilePage() {
  // Fetch the session inside the server component
  const session = await auth();

  // Redirect to home page if no session is found
  if (!session) {
    redirect("/");
  }
            ...
            <Tabs defaultValue="profile" className="w-full">
              ...
              <TabsContent value="appointments">
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      { session?.user?.appointments ? session?.user?.appointments.map((appointment, index) => (
                        <div
                          key={index}
                          className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-lg space-y-2 sm:space-y-0"
                        >
